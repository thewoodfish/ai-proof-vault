 ...
 */
async function storeOnFilecoin(proofObj, network) {
    ...
    try {
        const proofData = new TextEncoder().encode(JSON.stringify(proofObj, null, 2));
 ...
 */
async function setupPayments(synapse, depositAmount = 100) {
    ...
    const approveTx = await synapse.payments.approveService(warmStorageAddress, ethers.parseUnits("1", 18), // rate allowance
 ...
 */
async function retrieveFromFilecoin(pieceCid, network) {
    const rpcURL = getRpcURL(network);
    const synapse = await Synapse.create({
        privateKey: PRIVATE_KEY,
        rpcURL,
